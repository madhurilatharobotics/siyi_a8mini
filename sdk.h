...
class SIYI_SDK {
public:
    explicit SIYI_SDK(const char *ip_address = "192.168.144.25", int port = 37260);

    ~SIYI_SDK();
    void ptz_move(float pan, float tilt, float zoom);
    SIYI_Message msg;
    ...
    bool set_gimbal_angles(float pan, float tilt);
    ...
    [[nodiscard]] std::tuple<int, int> get_maximum_zoom() const;
