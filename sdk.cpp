...
void SIYI_SDK::ptz_move(float pan, float tilt, float zoom) {
    std::cout << "Moving PTZ: Pan=" << pan << " Tilt=" << tilt << " Zoom=" << zoom << std::endl;

    // Set gimbal angles (for Pan and Tilt)
    bool success = set_gimbal_angles(pan, tilt);
    if (!success) {
        std::cerr << "Failed to move Pan-Tilt" << std::endl;
    }
...
void SIYI_SDK::receive_message_loop(bool &connected) {
    while (connected) {
        ...
        for (int i = 0; i < bytes; i++) ss << std::setw(2) << (int) (unsigned char) buff[i];
        ...
        while (buff_str.length() >= MINIMUM_DATA_LENGTH) {
            ...
            // Check if we have enough data length, bytes are reversed, according to SIYI built-in SDK.
            std::string low_b = buff_str.substr(6, 2);
            std::string high_b = buff_str.substr(8, 2);
            std::string data_len_str = high_b + low_b;
            int data_len = std::stoi(data_len_str, nullptr, 16);
            ...
            if (buff_str.length() > (MINIMUM_DATA_LENGTH + data_len * 2)) {
                ...
                if (std::get<0>(decoded).empty()) continue;
...
void SIYI_SDK::ptz_move(float pan, float tilt, float zoom)
{
    // Move the gimbal to the requested angles
    bool success = set_gimbal_angles(pan, tilt);
    
    if (!success) {
        std::cerr << "Failed to move gimbal to (Pan: " << pan << ", Tilt: " << tilt << ")" << std::endl;
    }

    // Set the zoom level (assuming zoom is integer + fractional)
    int zoom_integer = static_cast<int>(zoom);
    int zoom_fractional = static_cast<int>((zoom - zoom_integer) * 10); // Convert decimal part

    success = set_absolute_zoom(zoom_integer, zoom_fractional);
    
    if (!success) {
        std::cerr << "Failed to set zoom level: " << zoom << std::endl;
    }
}
...
void SIYI_SDK::parse_maximum_zoom_msg(const std::string &parse_msg, int seq) {
    ...
    max_zoom_msg.integer = max_int;
    max_zoom_msg.fractional = max_float;
}
...
std::tuple<int, int> SIYI_SDK::get_maximum_zoom() const {
    return std::make_tuple(max_zoom_msg.integer, max_zoom_msg.fractional);
}
